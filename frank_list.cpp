...
{
    typename FrankList<T>::const_iterator it = ob.cbegin();
    while (it != nullptr) {
        out << *it << " ";
        ++it;
    }
...
{
    this->ptr = std::exchange(rhv->ptr, nullptr);
    return *this;
}
...
FrankList<T>::reverse_iterator::reverse_iterator(base_iterator&& rhv)
    :const_reverse_iterator(std::move(rhv.ptr))
{}
...
{
    if (this != &rhv){
        head = std::exchange(rhv.head, nullptr);
        ahead = std::exchange(rhv.ahead, nullptr);
        tail = std::exchange(rhv.tail, nullptr);
        atail = std::exchange(rhv.atail, nullptr);
    }
    return *this;
}
...
template <typename T>
void FrankList<T>::displaya() const
{
    Node* current =  ahead;
    while(current){
        std::cout << current -> val << " ";
